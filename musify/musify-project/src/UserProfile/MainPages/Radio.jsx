...
const Radio = () => {
  ...
  const handleSkipBackward = () => {
    audioRef.current.currentTime -= 10; // Skips back 10s
  };

  const handleSkipForward = () => {
    audioRef.current.currentTime += 10; // Skips forward 10s
  };
    ...
    <div className="w-full h-[calc(100vh-70px)] flex">
      ...
      <aside className="w-60 h-full bg-gray-900 text-white p-5 flex flex-col space-y-4 gap-5 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-900">
        ...
        <nav className="space-y-5">
          <p className="cursor-pointer hover:text-gray-400">🔥 New Releases</p>
          <p className="cursor-pointer hover:text-gray-400 flex items-center">
          ...
          <p className="cursor-pointer hover:text-gray-400">📻 Radio</p>
          <br />
                  ...
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    ...
                    >
                      {currentSong?.id === song.id && isPlaying ? (
                        <FaPause className="h-10 w-10 bg-black bg-opacity-50  p-2 rounded-full" />
                      ) : (
                        <FaPlay  className="h-10 w-10 bg-black bg-opacity-50 e p-2 rounded-full" />
                      )}
                    </button>
                  </div>
      ...
      {/* Music Player Footer */}
      <footer className="fixed bottom-0 left-0 w-full h-[70px] bg-gray-900 text-white px-5 flex items-center justify-between">
        <div className="flex items-center space-x-5 mx-auto">
          ...
          />
          <button onClick={togglePlayPause} className="bg-gray-700 p-3 rounded-full">
            {isPlaying ? <FaPause className="text-2xl h-8 w-8" /> : <FaPlay  className="text-2xl h-8 w-8" />}
          </button>
          <FaForward
            className="text-2xl cursor-pointer hover:text-gray-400"
        ...
        <div className="flex items-center space-x-4">
          ...
          />
          <FaExpand className="text-xl cursor-pointer hover:text-gray-400" />
        </div>
      </footer>
    </div>
  );
